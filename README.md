...
| Idle unload: models are *released* after a quiet stretch (Settings, default 15 min) — measured: RSS does not drop on its own (the allocator keeps the pages; macOS reclaims them under pressure), the model is simply gone and reloads on the next question | |
...
<!-- eval-ci:start -->
| repo docs+source subset (the CI run) / EmbeddingGemma int8 | 0.917 | 0.863 | 0.819 |
<!-- eval-ci:end -->
