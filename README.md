...
| Indexing: local files (native picker), **PDF / DOCX / PPTX text extraction** (scanned PDFs are skipped — no OCR yet), GitHub repos (device code / gh / token), GitLab repos (token), Confluence spaces (connect + CQL), Google Drive (connect + doc export), Microsoft OneDrive/SharePoint (device flow), Notion (shared pages), website crawl (same-host, HTML) | XLSX and audio; OCR for scanned PDFs; sidecar bundling in the DMG |
